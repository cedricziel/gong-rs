...
/// struct for passing parameters to the method [`integration_settings`]
#[derive(Clone, Debug)]
pub struct IntegrationSettingsParams {
    pub integration_settings_request: models::IntegrationSettingsRequest
}


/// struct for typed errors of method [`integration_settings`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum IntegrationSettingsError {
...
pub async fn integration_settings(configuration: &configuration::Configuration, params: IntegrationSettingsParams) -> Result<models::RegisterGenericCrmResponse, Error<IntegrationSettingsError>> {
    let local_var_configuration = configuration;

    // unbox the parameters
    let integration_settings_request = params.integration_settings_request;


    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v2/integration-settings", local_var_configuration.base_path);
    ...
    local_var_req_builder = local_var_req_builder.json(&integration_settings_request);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<IntegrationSettingsError> = serde_json::from_str(&local_var_content).ok();
