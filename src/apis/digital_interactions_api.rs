...
/// struct for passing parameters to the method [`add_digital_interaction`]
#[derive(Clone, Debug)]
pub struct AddDigitalInteractionParams {
    pub digital_interaction_request: models::DigitalInteractionRequest
}


/// struct for typed errors of method [`add_digital_interaction`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum AddDigitalInteractionError {
...
pub async fn add_digital_interaction(configuration: &configuration::Configuration, params: AddDigitalInteractionParams) -> Result<models::EventAcceptedResponse, Error<AddDigitalInteractionError>> {
    let local_var_configuration = configuration;

    // unbox the parameters
    let digital_interaction_request = params.digital_interaction_request;


    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v2/digital-interaction", local_var_configuration.base_path);
    ...
    local_var_req_builder = local_var_req_builder.json(&digital_interaction_request);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<AddDigitalInteractionError> = serde_json::from_str(&local_var_content).ok();
